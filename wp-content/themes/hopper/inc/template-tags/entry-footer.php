...
function entry_footer() {
  ...
  if ( 'post' === get_post_type() ) :
    ...
    $tags_list = get_the_tag_list( '', esc_attr_x( ', ', 'list item separator', 'hopper' ) );
    if ( $tags_list ) {
  ...
  if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
    ...
    comments_popup_link( sprintf( wp_kses( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'hopper' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
    echo '</span>';
  }
